...
> **Обновлено 2026-09-11 (Pre-release 0.6). Читать перед любыми изменениями.**
> Свежие правила — в AGENTS.md (снапшот + критические правила), детали
> экспериментов — в STRATEGY_TRIALS. Этот файл — базовые принципы.
>
...
### `repeats=N` — не ставить везде (обновлено 2026-09-11)
- **Т2: Discord требует repeats=7 минимум** (6→000, 7→200) — и с nodrop,
  и без. Причина: доля google-фейков в потоке (механика — STRATEGY_TRIALS,
  pktmon-захват).
- YouTube (google-блок): repeats=6. General TCP: repeats=8.
- QUIC в default: без repeats (при регрессе — 6-11).
- Для прочих сетей: **НЕ увеличивать «на всякий случай»**.
- Старое правило «nodrop + repeats=1» УСТАРЕЛО (было верно для сетей, где
  фейк работает сам; на Т2 повтор обязателен).
