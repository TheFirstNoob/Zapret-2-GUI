...
- **Кавычки - обычные** `"..."`. Кавычки-ёлочки не используем (в
  моношрифте они выглядят как `<>`, читатели путаются).
...
- **Длинные абзацы дробить**: списки `-`, короткие строки, подзаголовки.
  Одна сплошная строка с кучей символов читается плохо (частый отзыв).
- Жаргон расшифровывать при первом упоминании (например: "группа сервисов"
  вместо "семья", "работает нестабильно" вместо "флакает", "сразу все"
  вместо "батчем", блоб - "заготовка поддельного пакета"). Сложные термины
  собираются в "Словарь" (README).
- **В UI пунктуацию "оптом" НЕ менять.** Правила про кавычки и тире - для
  README и длинных текстов (моношрифт, где ёлочки похожи на `<>`). В
  интерфейсе короткие подписи, и там ёлочки/длинное тире - нормальная
  типографика. Тексты UI - только точечно, с визуальной проверкой
  (скриншот), когда реально мешает. UI уже дважды отрефакторен - аккуратно.