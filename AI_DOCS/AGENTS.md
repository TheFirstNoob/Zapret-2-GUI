...
>
> **Последнее обновление: 2026-08-27 (Pre-Release 0.4)**
...
работников — см. ниже). Готовый блок для hosts: **`hosts-github-fix.txt`**
в корне репо (основной IP 151.101.66.132 + запасные) — пользователи
просто добавляют его в hosts и делают flushdns. Скрипты-генераторы в
репо НЕ кладём (решение: отдавать сам файл hosts — чище и позволяет
обновлять его). Написанный ранее github-hosts-fix.ps1 удалён.
⚠️ Запись в hosts АТОМАРНО (temp + MoveFileEx): FileMode::Create на
живой файл ОПАСЕН — при аборте hosts усекается до 0 байт (случай 2026-08-30).
