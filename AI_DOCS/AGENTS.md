...
- **Пресеты: 8** (default + 7 тестовых). `lists/`: list-general содержит
...
- Мегафон-пользователь: дискорд пробился после lua-init фикса, ютуб нет —
  нужны его debug-лог/диагностика (§27-контекст; свежий exe 0.4).
- GUI portable (webview) ни разу не открывали руками (тестировались
  сервер/контроллер headless).
- VM-тест с облаком Защитника на новых сборках — рекомендован.
- Возможные фичи (обсуждены, не начаты): «Починить GitHub» кнопка в
  диагностике (Fastly-проба + атомарный hosts), вердикт «IP-range блок»
  в классификаторе, автоподбор desync-параметров, DNS-здоровье в
  диагностике. Пользователь обещал показать «конкурента» для идей (MIT).
...
## 28. Анализ zapret2UI (конкурент) + внедрённое (2026-08-30) 🚨

Конкурент: **zapret2UI (Asterlike)** — C# WPF .NET 9, single-file 166 MB,
MVVM. Его сила не в коде, а в цикле «кандидат → РЕАЛЬНАЯ проба winws2 →
скор → артефакт»: автоподбор стратегии (одноразовый движок-пробник,
готовность по строке лога «capture is started», параллельные TLS1.2/1.3/GET
пробы, взвешенный скор), генератор персональной стратегии (пул 24 TLS-
бандлов, скор Discord/YouTube отдельно + проверка собранного комбо целиком),
честный baseline (доступность БЕЗ обхода), память по сетям (fingerprint =
gateway IP + MAC + /24), трансформация аргументов при старте (токены +
QUIC-off/game-filter/scope). Слабое у него: 166 MB монолит (у нас 18/16.5/1.7),
хардкод доменного ноу-хау, TG-прокси/WARP-MASQUE = ЧУЖОЙ код (Flowseal/usque)
— в наш MIT-репо не тащим. Авто-лечение (TLS-проба каждые 45с) сознательно
НЕ берём: у нас autohostlist уже решает блоки, а постоянные TLS-стуки —
подозрительный паттерн для AV/игровых провайдеров.
...
- НЕ внедряли: Job Object (у нас запуск через bat — winws2 внук cmd, хрупко;
  убийства по имени image уже покрывают восстановление), память по сетям,
  автоподбор кандидатов, генератор стратегий — кандидаты на следующую итерацию.
