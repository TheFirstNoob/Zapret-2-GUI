...
>
> **Последнее обновление: 2026-08-30 (Pre-Release 0.6)**
> **Размер EXE:** 18.4 MB
...
- **VERSION = "Pre-Release 0.6"** (config.py + VERSION-файл + version_info 0.6.0.0).
...
### ВИСИТ (не сделано)
- **🔲 СИСТЕМНЫЙ АУДИТ UI (главная задача 2026-08-31+)** — пользователь
  указал: проблема не точечная, а системная — иерархия страниц, лишние
  блоки, разделители/«палочки», подчёркивания, где информация сходится,
  а где расходится. Пример уже починенного класса проблем: пост-тестовые
  рекомендации (3 кнопки запуска, вердикт «НЕ ПРОБИТО» при рабочем обходе,
  перемешивание живых строк разных стратегий). Аудит-план:
  1) каждая страница (Главная/Тестер/Списки/Диагностика): список блоков,
     что лишнее, что дублируется, где противоречия;
  2) единая иерархия (eyebrow → title → panel), единые отступы/типографика;
  3) все места «три действия на одну задачу» → одно действие;
  4) live-строки тестера: ГРУППИРОВКА ПО ПРОФИЛЯМ (сервер уже шлёт
     profile в results — коммит 0cf0985; фронт-группировка НЕ сделана);
  5) а11y: :focus-visible, prefers-reduced-motion, системные шрифты.
  Идеи из frontend2 (эксперимент другого агента): чип статуса защиты,
  live-валидация списков, вердикт-штамп сеткой — НЕ брать жёлтый акцент,
  капс, пульсации, demo.js. Вести через агента со скиллом frontend-design
  + живой обратной связью пользователя.
