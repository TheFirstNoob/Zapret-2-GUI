...
>
> **Последнее обновление: 2026-09-12 (Post-Release 0.6+; UI-рефактор ведёт отдельный агент)**
> **Размер EXE:** 18.5 MB (⚠ сборки НЕ пересобраны после 705f72b — перед релизом: build.py + build_portable.py + build_lite.py)
> **Статус:** `default.txt` — универсальная стратегия; Discord-блоки на repeats=8
> **Свежие находки 09-12:** Discord repeats=8 (утренний сдвиг порога ТСПУ: 7→000, ALT11=8 пробил); QUIC-класс тестера вне network_rate (статус «≈ QUIC», не красный); диагностика: HTTP-80 фолбэк + QUIC-квирки = warn/skip, пробы параллельно (~6с); AWS и госуслуги: ТСПУ SYN-дропит ПОДСЕТИ в голую (ICMP жив, TCP-443 мёртв) — не наша часть, WARP; служба: SCM-recovery + binPath-верификация + OEM-bat; статика: no-store + app.js?v=mtime (кэш WebView2).
...
- **Сборки устарели** (нет билда после 705f72b): перед релизом пересобрать все три.
...
- **Тестер (2026-09-12): QUIC-класс = www.youtube.com, youtu.be, i.ytimg.com,
  redirector.googlevideo.com, storage.googleapis.com — ИЗ network_rate
  исключены** (синхронно core.tester.QUIC_QUIRK_DOMAINS и frontend QUIRK_HOSTS).
  Их 000 — свойство TCP-пробы «чужим клиентом» под десинком, браузер ходит
  через QUIC: статус «≈ QUIC» (серый), не BLOCKED. Чипсы «не пробито» не
  содержат control-канарейки и QUIC-хостов. Best tie-break по net_ok_count
  (не ok_count — тот включает пинги: default/auto случайно менялись).
  Live-счётчики «Хосты» = rated-флаг от бэкенда (без прыжков 17/17→8/8).
  Финиш-сообщение: «Профиль завершён: сеть N/N (X%), всего проверок N».
  Мёртвые для нас пресеты (fake-only 0-12% на Т2) — НЕ мусор: данные для
  других сетей (правило 8), полный прогон не укорачивать.
- **Диагностика (2026-09-12): сетевые пробы параллельно** (10 потоков, этап
  ~6с). Для 000: HTTP-80 фолбэк (живой HTTP = «интернет работает», ok);
  QUIC-квирк-хосты → warn (не fail, «проверьте в браузере»); не-квирк-хосты →
  classify_block (sni_block = «движок обязан брать»). Discord-аплоад-хост
  ВЫЧЕРСНУТ из чеков (клиентский QUIC-путь curl не измеряет — вечный серый шум).
  Госуслуги-инцидент: ТСПУ SYN-дропит подсеть 213.59.x в голую (даже на
  устройствах без запрета/VPN — кросс-проверено пользователем) — это НЕ
  «стратегия ломает»: RU-канарейки (gosuslugi/vk/ya) в чипсы не попадают,
  детект «ломает живое» = canary жива + RU-хост мёртв.
- **Анализ приложения (probe) — отдельная страница** (сайдбар «Диагностика»);
  pktmon-захват НЕ запускается без UDP-портов процесса (иначе ALL-UDP мусор
  со всей машины); захват считает sent/recv → вердикт «UDP: отправлено N,
  входящих 0 — глушится, WARP»; домены (DNS-cache) в вердикте и таблицах.
- **Служба zapret2 = winws2.exe напрямую** (start=auto). После установки —
  SCM-recovery (sc failure: restart через 60с, 3 попытки); binPath
  ВЕРИФИЦИРУЕТСЯ после sc create (старые/сломанные Win10 могут молча исказить
  cmdline → установка откатывается с диагнозом); service-bat пишется в OEM
  (ascii падал на кириллице). «Стратегия ?» в статусе = controller не знал
  профиль службы — фикс (current_strategy заполняется при install/start).
- **Статика**: Cache-Control no-store + app.js?v=<mtime> в index.html —
  WebView2 кэш больше не топит правки фронтенда.
- **Спорные домены** — отдельная страница «Списки»-группы: тумблер «В обход»
  (правит list-include-user.txt) + живая curl-проба с вердиктом (жив в голую →
  держи выключенным; глушится → включи; мёртв даже с десинком → WARP).
  amazonaws.com: убрать/вернуть тумблером, НЕ правкой list-general.
- **Сборщики:** build.py (exe), build_portable.py (portable), build_lite.py (lite).
- **Тестер: 18 хостов** (RATED 11 + CONTROL 7). Блобы: селектор «Блоб фейка» +
  ручной приоритет (google → sochi_park → проверенные).
- **Служба zapret2 = winws2.exe НАПРЯМУЮ** (binPath = "\"...\winws2.exe\" args",
  start=auto). sc create/config через ВРЕМЕННЫЙ .bat.

### Правило честности измерений (2026-09-12, критично для тестера/диагностики) 🚨
Измерение «чужого TLS-клиента» (curl/openssl/stdlib) сквозь движок ≠ реальный
опыт: браузер/клиенты ходят через QUIC/свой TLS, google-блок (fake+multisplit)
рвёт сторонний ClientHello. Симптомы: «тестер говорит ютуб не работает» /
«диагностика красная» при полностью рабочем пользователе. Все подобные
измерения: (1) не в rate/верdict без пометки; (2) статус нейтральный (QUIC/
warn/skip), не красный; (3) HTTP-80 фолбэк для «интернет жив»; (4) реальный
опыт = браузер/клиент, просьба проверить там. UDP-путь: sent/recv разница
(«шлём, не получаем» = глушится) — из pkg-захвата.
...
2. **ИМЯ БЛОБА НЕ ДОЛЖНО НАЧИНАТЬСЯ С ЦИФРЫ** (2026-09-11) 🚨:
   `fake:blob=2gis_tls` → фейки МОЛЧА не отправляются (0 пакетов в захвате,
   только реальный ClientHello) → блок не обходится. dry-run валиден, баг
   виден только в pktmon-захвате. Переименование `2gis_tls`→`g2_tls` → 100%
   работает. Правило: blob-имена из цифр+букв начинать с буквы.
...
3. **Запись в hosts ТОЛЬКО атомарно** (temp + MoveFileEx). FileMode::Create
   ...
   ⚠️ ПОСЛЕ атомарной замены файла hosts из %TEMP% — ОБЯЗАТЕЛЬНО перевыдать
   ACL: `icacls hosts /grant "*S-1-5-20:(R)"` (NetworkService), иначе DNS
   Client МОЛЧА игнорирует hosts (читает его от NetworkService, у файла из
   temp нет права) — симптомы: пины «не работают» даже после ребута
   (случай 2026-08-31, upload-assets). Лучше — in-place дописывание
   (FileMode::Append), оно не трогает ACL и watcher.
...
- **Единственное исключение**: `build_lite.py` пишет `test.ps1` с
   ...
   - **.bat/.cmd — тоже ИСКЛЮЧЕНИЕ из «UTF-8 без BOM»**: cmd.exe читает батники
      в кодовой странице КОНСОЛИ (не в UTF-8). Кириллица в bat (проверено
      2026-09-04 на start.bat, cp866-файл сломался в UTF-8-консоли):
      ✓ `chcp 65001 >nul` первой строкой (ASCII) + файл UTF-8 без BOM + **CRLF** —
        работает в Windows Terminal/UTF-8-консолях;
      ✓ или чисто ASCII-сообщения (самый надёжный, как `_zapret_run.bat`);
      ✗ cp866-файл БЕЗ `chcp 866` — мусор в UTF-8-консоли;
      ✗ LF-переводы строк — батникам нужен только CRLF (LF ломает парсинг).
...
7. **Списки: точечность над широтой (2026-08-31)** — фильтровать только то,
   ...
   остаются точечно). Родительский домен — только когда заблокировано
   семейство/регион целиком (storage.googleapis.com — все steamcloud-*;
   amazonaws.com — осознанный v1-прецедент). github: часть бьётся, часть
   нет → родители без слияния (github.com, githubusercontent.com,
   githubassets.com...). Перед расширением взвесить: «покрыть весь домен»
   vs «точечно для стабильности»; комментарии в list-файлы не писать.
8. **Результаты тестов привязывать к провайдеру (2026-08-31)** —
   «не сработало на Т2» ≠ «мусор» (oob/syndata 30–46% на Т2 — кандидаты
   для других сетей). Ведём каталог `AI_DOCS/STRATEGY_TRIALS.md`; проигравший
   на одной сети пресет НЕ удаляем. `tcp_md5` в нашем lua работает
   (проверено 2026-08-31: tcpmd5-fake 100% на Т2, не хуже default).
...
- ✅ **СИСТЕМНЫЙ АУДИТ UI (2026-08-31, 0.5)** — сделано в сессии:
  ...
  5) user-листы инжектятся во ВСЕ hostlist-профили (см. §9 — семантика
     winws2 подтверждена исходниками v1.0.5).
...
### Правило (обновлено 2026-09-12: порог сдвинулся на 8)
- **Т2 (утреннее окно 2026-09-12): Discord требует repeats=8 минимум**
  (утром 7 → 000, ALT11 (1.9.6, repeats=8) пробил мгновенно; в днёвке 7 и 8
  оба давали 200 — порог плавает, 8 = запас). **Работает и с nodrop, и без** —
  объём google-фейков решает (механика pktmon: ТСПУ классифицирует по
  доминирующему SNI, доля ≥90% = пропуск).
...
## 9. User-листы: hostlist — ПО ПРОФИЛЯМ, а не «в конец» (2026-08-30, сверено с исходниками v1.0.2; актуально для v1.0.5) 🚨
...
переопределяет предыдущие» для v1.0.5 НЕВЕРНА.
...
- `_curl_test` — HEAD → GET-фолбэк (#14), **БЕЗ пер-хостовых ретраев** (000 фиксируется как есть). Спорные домены перепроверяются одним общим речеком в конце прогона (§29)
...
## 29. Речек спорных доменов — «заблокирован» vs «временно недоступен» (2026-09-04)

### Зачем
Старое правило §13 «без retries» отменено, но **не** в пользу пер-хостовых
повторов. На Т2/Skynet были ложные 000: ютуб открывался, а тестер показывал
000 (транзиентный спайк/дроп). Но бездумный ретрай каждого 000 дорог и не
отличает «не пробила эта стратегия» от «сеть моргнула». Дизайн:
- в `_curl_test` ретраев НЕТ — 000 фиксируется как есть;
- каждый пресет и так тестирует все домены заново: если следующая стратегия
  даёт >100, домен не в `blocked`;
- спорно ОДНО: RATED-домен («популярный, должен работать»), который не
  пробила ЛУЧШАЯ стратегия. Такие перепроверяются один раз в конце прогона.

### Как устроено
```
_curl_test: HEAD(-m 6с) → GET(-m 6с) → код≥100 → OK; «000» → BLOCKED (без повтора)
                                                          ↑ пер-хостовых ретраев НЕТ

test_profiles (server.py), в конце:
  contested = RATED_HOSTS ∩ {домены, не пробитые best-стратегией}
              \ quirk_skip (YouTube TCP/QUIC — их 000 ожидаем, вердикт уже ok)
  ОДИН общий повтор (naked, concurrency=8):
    повторился код ≥100 → «временно недоступен (ретест)» — ожил, уходит из чипов «не пробито»
    снова «000»          → «заблокирован» — подтверждён
```
`_recheck_contested(tester, best, rec, progress)` (server.py) возвращает
обновлённый `rec` с полем `recheck: {temporary:[], blocked:[]}` и дописанной
строкой в `message`. Логика — только на уровне отчёта, `network_rate`/скор
не меняются.

### Связанные правки честности отчёта
- **`blocked_domains` считается по ЛУЧШЕЙ стратегии** (`best.results`), а не
  объединением по всем пресетам: домен, который пробила другая стратегия, не
  должен числиться в «не пробито» (union вводил в заблуждение).
- **`quirk_skip`**: при активном YouTube-приколе (§17) домены
  www.youtube.com/redirector.googlevideo.com/i.ytimg.com/youtu.be НЕ попадают
  в речек и не помечаются «заблокирован» (их 000 — известный TCP-прикол, в
  браузере работает через QUIC).

### Почему не ловим редкие падения (пример)
Дискорд упал из-за косяка на стороне Дискорда — редко, нестабильно, поймать
нельзя. Если домен реально недоступен, речек подтвердит «заблокирован», и
пользователь при повторе результата отпишет — дальше смотрим debug-лог/где
режется. Цель речека — убрать из «не пробито» только транзиентные спайки,
которые моргнули и прошли.

### Стоимость
Только на спорных доменах (RATED, не пробитых лучшей стратегией). Один
параллельный повтор, naked; обычно секунды, worst-case полностью
заблокированного профиля +~5-10с.
