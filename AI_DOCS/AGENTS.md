...
работников — см. ниже). Скрипт `github-hosts-fix.ps1` (в корне репо):
тянет https://api.fastly.com/public-ip-list (при 403 — фолбэк-список),
пробует кандидатов через curl --resolve на VERSION-файл, выбирает лучший
по пингу, пишет hosts АТОМАРНО (temp + MoveFileEx — FileMode::Create на
живой файл ОПАСЕН: при аборте hosts усекается до 0 байт!).
