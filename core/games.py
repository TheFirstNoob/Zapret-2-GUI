...
def default_games() -> dict:
    ...
    return {"games": [{
        ...
        # Симптом у друзей: «бесконечный поиск сессии» - похоже на блок
        # первого UDP-коннекта (кейс Wardogs). План: «Анализ приложения» ->
        # цели (IP/порты) -> правило fake:payload=all на этих портах
        # (Epic обычно UDP 5222, 5795-5847) -> проверка, что сессия стартует.
        "udp": [],
    }]}
